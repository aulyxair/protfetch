...
from .processor import ProcessedProtein # For type hinting if needed, though fetcher primarily returns raw FASTA
...
def _entrez_retry_call(entrez_func, *args, retries=3, delay=5, **kwargs):
    ...
    for attempt in range(retries):
        try:
            ...
            return handle
        except (requests.exceptions.ConnectionError, requests.exceptions.Timeout) as e: # HTTP errors often wrapped by Biopython
            log.warning(f"Entrez call failed (Attempt {attempt + 1}/{retries}): {e}. Retrying in {delay}s...")
            ...
            if "HTTP Error" in str(e) or "NCBI" in str(e):
                 log.warning(f"Entrez call resulted in an error (Attempt {attempt + 1}/{retries}): {e}. Retrying in {delay}s...")
                 if attempt + 1 == retries:
                    log.error(f"Entrez call failed after {retries} attempts with NCBI error: {e}")
                    raise
                 time.sleep(delay)
...
def fetch_protein_fasta_for_gene(
    ...
    try:
        ...
        if not protein_ids_all:
            log.warning(f"Gene '{gene_symbol}': No linked Protein UIDs found for Gene UIDs {gene_ids}.")
            # Try a broader link if RefSeq yields nothing
            log.debug(f"Gene '{gene_symbol}': Retrying elink with broader scope (gene_protein).")
            time.sleep(1)
            handle_elink_broad = _entrez_retry_call(
                Entrez.elink, dbfrom="gene", db="protein", id=gene_ids, retries=retries, delay=5
            )
        ...
        for i in range(0, len(protein_ids_all), batch_size):
            ...
            if not handle_efetch: continue # Skip batch on error, or decide to fail all
