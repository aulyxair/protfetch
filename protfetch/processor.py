...
class ProcessedProtein:
    ...
    def __init__(
        ...
        self.gene_symbol_input = (
            gene_symbol_input.lower()
        )  # Store gene symbol in lowercase as in original
        self.identifier_from_header = (
            identifier_from_header  # This is the 'species' or specific ID from header
        )
...
def _parse_fasta_header(header_content: str) -> tuple[str | None, str | None, str]:
    ...
    if header_content.startswith("sp|") or header_content.startswith("tr|"):
        ...
        if len(parts) >= 2:
            ...
            if len(parts) >= 3:
                identifier = (
                    parts[2].split(None, 1)[0].strip()
                )  # First part of the description field
            ...
            accession = header_content.split(None, 1)[
                0
            ].strip()  # Get the first word as accession
            identifier = (
                accession  # Default identifier to accession if specific parsing fails
            )
    else:
        ...
        if start_bracket != -1 and end_bracket != -1 and end_bracket > start_bracket:
            ...
            if len(parts) > 1:
                ...
                identifier = (
                    accession  # Default to accession if no specific identifier found
                )
            else:
                identifier = accession
...
def process_fasta_stream(
    ...
    for record in SeqIO.parse(fasta_stream, "fasta"):
        ...
        raw_header_content = (
            record.description
        )  # Biopython's record.description is the full header line after '>'
    ...
    for i in range(n_candidates):
        ...
        for j in range(
            n_candidates
        ):  # Compare with all others (including those already processed as protein1)
            if i == j:
                continue
...
def write_processed_proteins_to_csv(
    ...
    try:
        with open(output_file_path, "w", newline="") as outfile_csv:
            ...
            csv_writer.writerow(
                ["accession", "gene_input", "identifier_from_header"]
            )  # Header row
            for protein in proteins:
