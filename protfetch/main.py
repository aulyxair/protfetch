...
from . import utils
from .utils import (
    log, setup_logging, parse_gene_list_file, GeneInput, ensure_output_dir,
    DEFAULT_MAX_DIST, DEFAULT_ENTREZ_EMAIL, DEFAULT_REQUEST_TIMEOUT,
    DEFAULT_REQUEST_RETRIES, DEFAULT_MAX_WORKERS, OUTPUT_SUBDIR_INDIVIDUAL,
    COMBINED_FASTA_SHORT_SUFFIX, COMBINED_FASTA_FULL_SUFFIX, COMBINED_CSV_SUFFIX
)
from .fetcher import configure_entrez, fetch_protein_fasta_for_gene, filter_fasta_by_keyword
from .processor import (
    process_fasta_stream, ProcessedProtein,
    write_processed_proteins_to_fasta, write_processed_proteins_to_csv,
...
def process_single_gene_task(
    ...
    if not skip_keyword_filter:
        ...
        if not keyword_filtered_fasta_content:
            log.warning(f"No sequences remained for gene {gene_symbol} after keyword filtering with '{gene_input.query_keyword}'.")
    ...
    if not keyword_filtered_fasta_content.strip(): # If empty after filtering or fetch
        log.info(f"No FASTA content to process for gene {gene_symbol} after fetch/keyword filter.")
    ...
    if not processed_proteins:
        ...
        log.info(f"Successfully processed gene {gene_symbol}, kept {len(processed_proteins)} proteins.")
...
def main_workflow(args):
    ...
    num_workers = min(args.max_workers, len(genes_to_process)) # Don't use more workers than tasks
        ...
        for future in tqdm(as_completed(future_to_gene_input), total=len(genes_to_process), desc="Processing genes"):
            ...
            try:
                ...
                if result:
                    ...
                    log.error(f"No result returned for gene: {gene_input_obj.gene_symbol}")
                    all_stats.append({"gene_symbol": gene_input_obj.gene_symbol, "error": "Task failed unexpectedly"})
            except Exception as e:
                log.error(f"Error processing gene {gene_input_obj.gene_symbol} in worker: {e}", exc_info=True)
                all_stats.append({"gene_symbol": gene_input_obj.gene_symbol, "error": str(e)})
    ...
    if not all_processed_proteins:
        ...
        log.info(f"Combining results from {len(all_processed_proteins)} initially collected proteins...")
        ...
        dedup_by_accession_list.sort(key=lambda p: p.accession) # Sort for consistent output
    ...
    successful_genes = 0
    failed_genes = 0
    total_proteins_final_in_combined = len(dedup_by_accession_list) if all_processed_proteins else 0
    ...
    for gene_stat_summary in all_stats:
        ...
        if "error" in gene_stat_summary:
        ...
        elif "status" in gene_stat_summary and gene_stat_summary["status"] == "No content post-filter":
            log.info(f"  Gene {gs}: Processed, but no content after initial fetch/filter.")
            ...
            log.info(f"  Gene {gs}: Processed, {kept_count} sequences kept from this gene.")
            if kept_count > 0 or gene_stat_summary.get('initial_unique_sequences_parsed', 0) > 0 : # Count as success if any parsing happened
                 successful_genes +=1
    ...
    log.info(f"  Successfully processed (or processed with no data): {successful_genes}")
    log.info(f"  Failed during processing: {failed_genes}")
    log.info(f"Total unique proteins in combined outputs (dedup by accession): {total_proteins_final_in_combined}")
    
    end_time = time.time()
    log.info(f"Total execution time: {end_time - start_time:.2f} seconds.")
    log.info("--- End of Summary ---")
    
    if failed_genes > 0 and successful_genes == 0 : return 1 # Error if all failed
    return 0
...
def cli_entry():
    ...
    parser.add_argument(
        "-v", "--version",
        action="version",
        version=f"%(prog)s {utils.__version__}"
    )
