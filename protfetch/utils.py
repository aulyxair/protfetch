...
def setup_logging(level=logging.INFO):
    ...
    # Add ch to logger
    if not logger.handlers: # Avoid adding multiple handlers on re-runs (e.g., in tests)
...
class GeneInput:
    ...
    def __init__(self, line: str):
        ...
        self.protein_name: str | None = None # From input like "Protein Name | GENE_SYMBOL"
        ...
        if '|' in self.original_line:
            ...
            self.query_keyword = self.protein_name # Use full protein name as keyword by default
        else:
            self.gene_symbol = self.original_line
            self.query_keyword = self.gene_symbol # Default keyword is the gene symbol itself

        if not self.gene_symbol:
            raise ValueError(f"Could not parse gene symbol from line: '{self.original_line}'")
        if not self.query_keyword: # Should not happen if gene_symbol is present
             raise ValueError(f"Could not determine query keyword for line: '{self.original_line}'")
    ...
    try:
        with open(file_path, 'r') as f:
            for i, line_content in enumerate(f, 1):
                ...
                if not line_content or line_content.startswith('#'): # Skip empty lines and comments
                    continue
                try:
                    genes_to_process.append(GeneInput(line_content))
                except ValueError as e:
                    log.warning(f"Skipping invalid line {i} in '{file_path}': {line_content}. Error: {e}")
