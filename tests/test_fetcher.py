...
from Bio import Entrez # Crucial import for Entrez
from protfetch.fetcher import fetch_protein_fasta_for_gene, configure_entrez, _entrez_retry_call
from protfetch.utils import GeneInput

@pytest.fixture(autouse=True)
def auto_configure_entrez_for_tests():
    configure_entrez("ci_test@example.com")
@patch('protfetch.fetcher.time.sleep') # Mock time.sleep to speed up tests
@patch('protfetch.fetcher.Entrez.efetch')
@patch('protfetch.fetcher.Entrez.elink')
@patch('protfetch.fetcher.Entrez.esearch')
def test_fetch_protein_fasta_for_gene_success(mock_esearch, mock_elink, mock_efetch, mock_sleep):
    
    mock_esearch_handle = MagicMock()

    with patch('Bio.Entrez.read', MagicMock(return_value={'IdList': ['12345']})) as mock_entrez_read_esearch:
        mock_esearch.return_value = mock_esearch_handle 

        mock_elink_handle = MagicMock()
        with patch('Bio.Entrez.read', MagicMock(return_value=[{'LinkSetDb': [{'Link': [{'Id': 'NP_001'}, {'Id': 'XP_002'}]}]}])) as mock_entrez_read_elink:
            mock_elink.return_value = mock_elink_handle # elink returns a handle

            # Mock for efetch
            mock_efetch_handle = MagicMock()
            mock_efetch_handle.read.return_value = ">NP_001 Protein1\nACGT\n>XP_002 Protein2\nTGCA"
            mock_efetch.return_value = mock_efetch_handle # efetch returns a handle
...
@patch('protfetch.fetcher.time.sleep') # Mock time.sleep
@patch('protfetch.fetcher.Entrez.esearch')
def test_fetch_protein_fasta_no_gene_uid(mock_esearch, mock_sleep):
    mock_esearch_handle = MagicMock()
    with patch('Bio.Entrez.read', MagicMock(return_value={'IdList': []})) as mock_entrez_read: 
        mock_esearch.return_value = mock_esearch_handle
...
def test_entrez_retry_call_failure(mock_esearch, mock_elink_fails, mock_sleep):
    ...
    with patch('Bio.Entrez.read', MagicMock(return_value={'IdList': ['12345']})) as mock_entrez_read_esearch:
        ...
        assert fasta_content is None
        assert mock_elink_fails.call_count == 2 # Called once, retried once (total 2 attempts for retries=2)
