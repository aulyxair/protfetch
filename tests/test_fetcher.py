...
def test_fetch_protein_fasta_for_gene_success(mock_esearch, mock_elink, mock_efetch, mock_sleep):
    ...
    with patch('Bio.Entrez.read', MagicMock(return_value={'IdList': ['12345']})) as mock_entrez_read_esearch_context:
        mock_esearch.return_value = mock_esearch_handle

        mock_elink_handle = MagicMock()
        with patch('Bio.Entrez.read', MagicMock(return_value=[{'LinkSetDb': [{'Link': [{'Id': 'NP_001'}, {'Id': 'XP_002'}]}]}])) as mock_entrez_read_elink_context:
